...
HOST=${LS_HOST:-http://localhost:8420}
SK=sk_test_12345

testProductSeededOnStartup=$(curl -sSfg -u $SK: $HOST/v1/products | grep -oE 'prod_testSeedOnStartUp' | head -n 1)
[ "$testProductSeededOnStartup" = "prod_testSeedOnStartUp" ]

testCouponSeededOnStartup=$(curl -sSfg -u $SK: $HOST/v1/coupons | grep -oE 'coupon_testSeedOnStartUp' | head -n 1)
[ "$testCouponSeededOnStartup" = "coupon_testSeedOnStartUp" ]

testPlanSeededOnStartup=$(curl -sSfg -u $SK: $HOST/v1/plans | grep -oE 'plan_testSeedOnStartUp' | head -n 1)
