...
testProductSeededOnStartup=$(curl -sSfg -u $SK: $LS_HOST/v1/products | grep -oE 'prod_testSeedOnStartUp' | head -n 1)
[ "$testProductSeededOnStartup" = "prod_testSeedOnStartUp" ]

testCouponSeededOnStartup=$(curl -sSfg -u $SK: $LS_HOST/v1/coupons | grep -oE 'coupon_testSeedOnStartUp' | head -n 1)
[ "$testCouponSeededOnStartup" = "coupon_testSeedOnStartUp" ]

testPlanSeededOnStartup=$(curl -sSfg -u $SK: $LS_HOST/v1/plans | grep -oE 'plan_testSeedOnStartUp' | head -n 1)
[ "$testPlanSeededOnStartup" = "plan_testSeedOnStartUp" ]

testPriceSeededOnStartup=$(curl -sSfg -u $SK: $LS_HOST/v1/prices | grep -oE 'price_testSeedOnStartUp' | head -n 1)
[ "$testPriceSeededOnStartup" = "price_testSeedOnStartUp" ]

curl -X DELETE $LS_HOST/_config/data

cus=$(curl -sSfg -u $SK: $LS_HOST/v1/customers \
          -d email=james.robinson@example.com \
      | grep -oE 'cus_\w+' | head -n 1)

curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus \
     -d description='Adding a description...'

curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus \
     -d preferred_locales[]='fr-FR' -d preferred_locales[]='es-ES'

cusB=$(curl -sSfg -u $SK: $LS_HOST/v1/customers \
            -d email=george.carlin@example.com \
       | grep -oE 'cus_\w+' | head -n 1)

email=$(curl -sSfg -u $SK: $LS_HOST/v1/customers?email=james.robinson@example.com \
        | grep -oE '"email": "james.robinson@example.com",')
[ -n "$email" ]

curl -sSfg -u $SK: -X DELETE $LS_HOST/v1/customers/$cus
curl -sSfg -u $SK: -X DELETE $LS_HOST/v1/customers/$cusB
...
curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/tax_ids \
     -d type=eu_vat \
     -d value=DE123456789 \
     -d expand[]=customer

tax_id=$(curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/tax_ids \
              -d type=eu_vat \
              -d value=BE00111222333 \
         | grep -oE 'txi_\w+' | head -n 1)

curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus?expand[]=tax_ids.data.customer

curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus?expand[]=subscriptions.data.items.data

code=$(curl -sg -o /dev/null -w '%{http_code}' -u $SK: \
       $LS_HOST/v1/customers/$cus?expand[]=subscriptions.data.items.data.tax_ids)
[ "$code" -eq 400 ]

curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/tax_ids/$tax_id

curl -sSfg -u $SK: -X DELETE $LS_HOST/v1/customers/$cus/tax_ids/$tax_id

code=$(curl -sg -o /dev/null -w '%{http_code}' -u $SK: \
       $LS_HOST/v1/customers/$cus/tax_ids/$tax_id)
[ "$code" -eq 404 ]

txr1=$(curl -sSfg -u $SK: $LS_HOST/v1/tax_rates \
            -d display_name=VAT \
...
txr2=$(curl -sSfg -u $SK: $LS_HOST/v1/tax_rates \
            -d display_name=VAT \
...
curl -sSfg -u $SK: $LS_HOST/v1/plans \
...
curl -sSfg -u $SK: -X DELETE $LS_HOST/v1/plans/delete-me

code=$(curl -sg -o /dev/null -w '%{http_code}' -u $SK: $LS_HOST/v1/plans \
            -d doesnotexist=1)
[ "$code" -eq 400 ]
...
curl -sSfg -u $SK: $LS_HOST/v1/products \
     -d name=T-shirt \
     -d type=good \
...
curl -sSfg -u $SK: $LS_HOST/v1/products/PRODUCT1234

curl -sSfg -u $SK: $LS_HOST/v1/plans?expand[]=data.product

count=$(curl -sSfg -u $SK: $LS_HOST/v1/products?active=false \
        | grep -oP 'total_count": \K([0-9]+)')
[ "$count" -eq 0 ]

count=$(curl -sSfg -u $SK: $LS_HOST/v1/products?active=true \
        | grep -oP 'total_count": \K([0-9]+)')
[ "$count" -eq 9 ]

code=$(curl -sg -o /dev/null -w '%{http_code}' -u $SK: \
            $LS_HOST/v1/plans?expand[]=data.doesnotexist)
[ "$code" -eq 400 ]

count=$(curl -sSfg -u $SK: $LS_HOST/v1/plans?active=false \
        | grep -oP 'total_count": \K([0-9]+)')
[ "$count" -eq 0 ]

count=$(curl -sSfg -u $SK: $LS_HOST/v1/plans?active=true \
        | grep -oP 'total_count": \K([0-9]+)')
[ "$count" -eq 6 ]

curl -sSfg -u $SK: $LS_HOST/v1/coupons \
   -d id=PARRAIN \
   -d percent_off=30 \
   -d duration=once

# This is what a Stripe.js request does:
tok=$(curl -sSfg $LS_HOST/v1/tokens \
...
curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/sources \
     -d source=$tok

# This is what a request from back-end does:
tok=$(curl -sSfg -u $SK: $LS_HOST/v1/tokens \
           -d card[number]=4242424242424242 \
           -d card[exp_month]=12 \
...
# add a new card
card=$(
  curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/cards \
...
count=$(curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/sources?object=card \
        | grep -oP 'total_count": \K([0-9]+)')
[ "$count" -eq 3 ]

# delete the card
curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/sources/$card \
     -X DELETE
...
card=$(curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/cards \
          -d source[object]=card \
          -d source[number]=4242424242424242 \
          -d source[exp_month]=12 \
          -d source[exp_year]=2020 \
...
captured=$(
  curl -sSfg -u $SK: $LS_HOST/v1/charges \
       -d customer=$cus \
       -d source=$card \
       -d amount=1000 \
       -d currency=usd \
...
captured=$(
  curl -sSfg -u $SK: $LS_HOST/v1/charges/$charge \
...
captured=$(
  curl -sSfg -u $SK: $LS_HOST/v1/charges/$charge/capture \
       -d amount=800 \
  | grep -oE '"captured": true,')
[ -n "$captured" ]
...
sepa_cus=$(
  curl -sSfg -u $SK: $LS_HOST/v1/customers \
       -d description='I pay with SEPA debit' \
       -d email=sepa@euro.fr \
  | grep -oE 'cus_\w+' | head -n 1)

src=$(curl -sSfg -u $SK: $LS_HOST/v1/sources \
           -d type=ach_credit_transfer \
           -d currency=usd \
           -d owner[email]='jenny.rosen@example.com' \
      | grep -oE 'src_\w+')

curl -sSfg -u $SK: $LS_HOST/v1/customers/$sepa_cus/sources \
     -d source=$src
...
curl -sSfg -u $SK: $LS_HOST/v1/customers/$sepa_cus/sources/$src
code=$(curl -sg -o /dev/null -w "%{http_code}" -u $SK: \
...
cus=$(curl -sSfg -u $SK: $LS_HOST/v1/customers -d email=joe.malvic@example.com \
      | grep -oE 'cus_\w+' | head -n 1)
ds=$(curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus?expand[]=default_source \
...
curl -sSfg -u $SK: $LS_HOST/v1/invoices?customer=$cus

code=$(curl -sg -o /dev/null -w "%{http_code}" -u $SK: \
            $LS_HOST/v1/invoices/upcoming?customer=$cus)
[ "$code" = 404 ]

curl -sSfg -u $SK: $LS_HOST/v1/subscriptions \
     -d customer=$cus \
     -d items[0][plan]=basique-mensuel \
     -d expand[]=latest_invoice.payment_intent.latest_charge

res=$(curl -sSfg -u $SK: $LS_HOST/v1/subscriptions \
           -d customer=$cus \
           -d items[0][plan]=basique-mensuel \
...
curl -sSfg -u $SK: $LS_HOST/v1/invoices/upcoming?customer=$cus

curl -sSfg -u $SK: $LS_HOST/v1/invoices/upcoming?customer=$cus\&subscription_items[0][plan]=pro-annuel\&subscription_tax_percent=20

curl -sSfg -u $SK: $LS_HOST/v1/invoices/upcoming?customer=$cus\&subscription=$sub\&subscription_items[0][id]=si_RBrVStcKDimMnp\&subscription_items[0][plan]=basique-annuel\&subscription_proration_date=1504182686\&subscription_tax_percent=20

curl -sSfg -u $SK: $LS_HOST/v1/invoices/$in/lines
...
curl -sSfg -u $SK: $LS_HOST/v1/subscriptions?customer=$cus

curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus/subscriptions
...
sub=$(curl -sSfg -u $SK: $LS_HOST/v1/subscriptions \
           -d customer=$cus \
           -d items[0][plan]=monthly-tiered-graduated \
           -d items[0][quantity]=5 \
      | grep -oE 'sub_\w+' | head -n 1)

data=$(curl -sSfg -u $SK: $LS_HOST/v1/subscriptions/$sub \
            -d items[0][plan]=annual-tiered-volume)

same_data=$(curl -sSfg -u $SK: $LS_HOST/v1/subscriptions/$sub \
                 -d items[0][plan]=annual-tiered-volume)

diff <(echo "$data") <(echo "$same_data")

curl -sSfg -u $SK: $LS_HOST/v1/subscriptions/$sub \
...
sub=$(curl -sSfg -u $SK: $LS_HOST/v1/subscriptions/$sub_id \
           -d items[0][plan]=pro-annuel \
           -d items[0][id]=$sub_item_id)

in=$(curl -sSfg -u $SK: $LS_HOST/v1/invoices \
          -d customer=$cus)
...
pm=$(curl -sSfg -u $SK: $LS_HOST/v1/payment_methods \
          -d type=card \
          -d card[number]=4242424242424242 \
          -d card[exp_month]=12 \
          -d card[exp_year]=2020 \
...
curl -sSfg -u $SK: $LS_HOST/v1/payment_methods/$pm/attach \
     -d customer=$cus

curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus \
     -d invoice_settings[default_payment_method]=$pm

curl -sSfg -u $SK: $LS_HOST/v1/customers/$cus?expand[]=invoice_settings.default_payment_method

curl -sSfg -u $SK: $LS_HOST/v1/payment_methods?customer=$cus\&type=card

curl -sSfg -u $SK: $LS_HOST/v1/payment_methods/$pm/detach -X POST
...
res=$(curl -sSfg -u $SK: $LS_HOST/v1/setup_intents -X POST)
seti=$(echo "$res" | grep '"id"' | grep -oE 'seti_\w+' | head -n 1)
seti_secret=$(echo $res | grep -oE 'seti_\w+_secret_\w+' | head -n 1)

curl -sSfg -u $SK: $LS_HOST/v1/setup_intents/$seti/confirm -X POST

curl -sSfg -u $SK: $LS_HOST/v1/setup_intents/$seti/cancel -X POST
...
# This is what a Stripe.js request does:
curl -sSfg $LS_HOST/v1/setup_intents/$seti/confirm \
     -d key=pk_test_sldkjflaksdfj \
...
fingerprint=$(
  curl -sSfg -u $SK: $LS_HOST/v1/sources \
       -d type=sepa_debit \
       -d sepa_debit[iban]=DE89370400440532013000 \
       -d currency=eur \
...
fingerprint=$(
  curl -sSfg -u $SK: $LS_HOST/v1/payment_methods \
       -d type=card \
       -d card[number]=4242424242424242 \
       -d card[exp_month]=12 \
       -d card[exp_year]=2020 \
...
charge=$(
  curl -sg -u $SK: $LS_HOST/v1/charges \
       -d source=$card \
       -d amount=1000 \
       -d currency=usd \
  | grep -oE 'ch_\w+' | head -n 1)
...
# list charges
total_count=$(
  curl -sSfg -u $SK: $LS_HOST/v1/charges | grep -oE '"total_count": 15,')
[ -n "$total_count" ]

total_count=$(
  curl -sSfg -u $SK: $LS_HOST/v1/charges?customer=$cus \
  | grep -oE '"total_count": 6,')
[ -n "$total_count" ]

total_count=$(
  curl -sSfg -u $SK: $LS_HOST/v1/charges?customer=$cus\&created[gt]=1588166306 \
  | grep -oE '"total_count": 6,')
[ -n "$total_count" ]

no_more_events=$(curl -sSfg -u $SK: $LS_HOST/v1/events \
                 | grep -oE '^  "has_more": false' || true)
[ -z "$no_more_events" ]
last_event=$(curl -sSfg -u $SK: $LS_HOST/v1/events?limit=100 \
             | grep -oE 'evt_\w+' | tail -n 1)
no_more_events=$(curl -sSfg -u $SK: $LS_HOST/v1/events?starting_after=$last_event \
                 | grep -oE '^  "has_more": false')
[ -n "$no_more_events" ]
zero_events=$(curl -sSfg -u $SK: $LS_HOST/v1/events?starting_after=$last_event \
                 | grep -oE '^  "data": \[\]')
[ -n "$zero_events" ]

curl -sSfg -u $SK: $LS_HOST/v1/balance

payout=$(
  curl -sSfg -u $SK: $LS_HOST/v1/payouts \
       -d amount=1100 \
       -d currency=eur \
  | grep -oE 'po_\w+' | head -n 1)

payout_status=$(
  curl -sSfg -u $SK: $LS_HOST/v1/payouts/$payout \
  | grep -oE '"status": "pending",')
[ -n "$payout_status" ]

curl -sg -u $SK: $LS_HOST/v1/payouts/$payout/cancel -X POST
...
curl -sg -u $SK: $LS_HOST/v1/payouts \
      -d amount=1100 \
      -d currency=eur \
      -d status=paid
...
src=$(
  curl -sSfg -u $SK: $LS_HOST/v1/balance_transactions/$txn \
  | grep -oE "$charge")
[ -n "$src" ]

# refund has a balance transaction
txn=$(
  curl -sSfg -u $SK: $LS_HOST/v1/refunds \
       -d charge=$charge \
  | grep -oE 'txn_\w+')
[ -n "$txn" ]
...
card=$(curl -sSfg -u $SK: $LS_HOST/v1/payment_methods?customer=$cus\&type=card \
      | grep -oE "$pm" | head -n 1)

[ -n "$card" ]

# trying to create a customer with a non-existant payment_method returns a 404, and doesn't create customer
total_count=$( curl -sSfg -u $SK: $LS_HOST/v1/customers \
             | grep -oE '"total_count": 9,')
[ -n "$total_count" ]

code=$(curl -sg -o /dev/null -w '%{http_code}' -u $SK: $LS_HOST/v1/customers \
...
amount=$(curl -sSfg -u $SK: $LS_HOST/v1/subscriptions \
              -d customer=$cus \
              -d items[0][plan]=basique-mensuel \
...
charge=$(curl -sSfgG -u $SK: $LS_HOST/v1/invoices \
              -d customer=$cus \
